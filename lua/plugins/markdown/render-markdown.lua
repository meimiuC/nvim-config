...
local function pad_to_eol(str)
    ...
    if spaces_needed > 0 then return str .. string.rep(' ', spaces_needed) else return str end
end
...
local function fold_virt_text(result, start_text, lnum)
    ...
    local ts_hl_str = captured_highlights[#captured_highlights] and
        ('@' .. captured_highlights[#captured_highlights].capture .. '.markdown') or ""

    if ts_hl_str ~= "" then
        local ok, hl = pcall(vim.api.nvim_get_hl, 0, { name = ts_hl_str, link = true })
        if ok and hl.link then ts_hl_str = hl.link end
    end
...
return {
    ...
    opts = {
        ...
        callout = {
            ...
            warning   = { raw = "[!WARNING]", rendered = "󰀪 Warning", highlight = "RenderMarkdownWarn", category = "github" }, -- GitHub 原生支持
            -- ── 引用类（灰色）────────────────────────────────────────────
        ...
        checkbox = {
            ...
            custom = {
                question  = { raw = "[?]", rendered = "󰋗", highlight = "RenderMarkdownError", scope_highlight = "RenderMarkdownError" }, -- 疑问
                todo      = { raw = "[>]", rendered = "󰦖", highlight = "RenderMarkdownInfo", scope_highlight = "RenderMarkdownInfo" }, -- 进行中
                canceled  = { raw = "[-]", rendered = "󰅘", highlight = "RenderMarkdownCodeFallback", scope_highlight = "@text.strike" }, -- 已取消（带删除线）
                important = { raw = "[!]", rendered = "󰳦", highlight = "RenderMarkdownWarn", scope_highlight = "RenderMarkdownWarn" }, -- 重要
                favorite  = { raw = "[~]", rendered = "󰓎", highlight = "RenderMarkdownMath", scope_highlight = "RenderMarkdownMath" }, -- 收藏
            },
        },
