-- ~/.config/nvim/lua/plugins/editor_enforcing/which-key.lua
return {
	...
	config = function(_, opts)
		...
		wk.add({
			...
			{ "<leader>bp", desc = "挑选标签页 (Pick)" },
			{ "<leader>bc", desc = "关闭当前标签页" },
			{ "<leader>bo", desc = "关闭其他所有标签页" },
			{ "<leader>br", desc = "关闭右侧所有标签页" },
			{ "<leader>c", group = "代码" },
			{ "<leader>d", group = "诊断浮窗" },
			{ "<leader>e", group = "文件管理 (Yazi)" },
			{ "<leader>f", group = "Snacks Picker" },
			{ "<leader>m", group = "Markdown" },
			{ "<leader>o", group = "Overseer / 任务" },
			{ "<leader>q", group = "诊断列表" },
			{ "<leader>r", group = "重命名/重构" },
			-- { "<leader>s", group = "搜索 (Telescope)" },
			-- 为终端 / 任务增加提示
			{ "<leader>t", group = "终端 / Tasks" },
			{ "[d", desc = "上一个诊断" },
			{ "]d", desc = "下一个诊断" },
